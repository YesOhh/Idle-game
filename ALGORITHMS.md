...
#### 极（通用可进化技能）
- 解锁：雇佣即解锁
- 效果：升级攻击力时，提升的攻击力额外增加120%，但每级攻击力升级降低0.5%攻速
- 公式：
  - `每次升级增加值 = floor(原增加值 × 2.2)`（里程碑奖励不受此加成）
  - `攻速惩罚 = attackInterval × 1.005^damageLevel`（按攻击力等级累计）
